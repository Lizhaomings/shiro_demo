package com.demo.shiro_demo.shiro.realm;
...
import com.demo.shiro_demo.shiro.utils.ShiroByteSource;
...
public class ShiroRealm extends AuthorizingRealm {
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principals) {
        ...
        if (!CollectionUtils.isEmpty(user.getRoleInfoList())) {
            roleList = user.getRoleInfoList().stream().map(RoleInfo::getRole)
                    .collect(Collectors.toList());
            info.addRoles(roleList);
        }
        List<String> resList;
        if (roleList.contains("admain")) {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken token) throws AuthenticationException {
        ...
        // 防止泄露， shiro 将 SimpleAuthenticationInfo的第一个参数存入标签中
